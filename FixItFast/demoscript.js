...
registrationForm.addEventListener("submit", (e) => {
  ...
  let serviceProviderId; // Declare serviceProviderId variable

  // Save the service provider data to Firestore
  db.collection("pendingUsers")
    .add(serviceProvider)
    .then((docRef) => {
      serviceProviderId = docRef.id; // Assign the value to serviceProviderId

      // Store the image in Firebase Storage
      const storageRef = firebase.default.storage().ref();
      const imageRef = storageRef.child(`images/${serviceProviderId}`);
    ...
    .then((downloadURL) => {
      ...
      // Update the service provider document with the download URL
      return db.collection("pendingUsers").doc(serviceProviderId).update({
